package com.sxmbit.library.internal;
...
public class IcsLinearLayout extends LinearLayout
{
    ...
    private static final boolean IS_HONEYCOMB = Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB;//HONEYCOMB Android 3.1ϵͳ
    ...
    public void setDividerDrawable(Drawable divider)
    {
        ...
        /**
         *MyLinearLayout从LinearLayout派生出来，然后在程序中重载OnDraw(Canvas canvas)。但是，onDraw不会被调用。
         * 我们可能会遇到这个问题：如果不给LinearLayout设置一个背景，系统是不会调用onDraw时，也就是说，我们重写的onDraw是不会调用的。
         * 当设置一个背景后，onDraw就会被调用。
         * 造成这种现象的原因是继承自LinearLayout，而LinearLayout这是一个容器，ViewGroup嘛，它本身并没有任何可画的东西，
         * 它是一个透明的控件，因些并不会触发onDraw，但是你现在给LinearLayout设置一个背景色，其实这个背景色不管你设置成什么颜色，
         * 系统会认为，这个LinearLayout上面有东西可画了，因此会调用onDraw方法。
    ...
    protected void measureChildWithMargins(View child, int parentWidthMeasureSpec, int widthUsed, int parentHeightMeasureSpec, int heightUsed)
    {
        ...
        if (index == count - 1)
        {
            if (hasDividerBeforeChildAt(count))//估计是分割线
            {
                if (orientation == VERTICAL)
                {
